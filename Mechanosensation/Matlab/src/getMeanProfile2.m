function [ Mean, Std, N, Profiles, FluoMean, CellWidth, CellLength, CellID, Fluo_Flip_signal] = getMeanProfile2( Path, NbUpSamples, FluoProt, Cell_Length )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here
    %% Loading data
    DataSet=load(Path);
    PixSize=str2double(DataSet.params.Scaling.String);
    FieldName=strcat("MedialAxisIntensity_",FluoProt);
    ...
    Fluo_Flip_signal=zeros(totKeptCells, 1);
    ...
    for h=1:NbFrames
        for i=1:NbCells(h)
            if((~DataSet.frames(h).cells.Stats(i).CellDeleted) && (DataSet.frames(h).cells.Stats(i).CellLength)< LenInPix)
                ...
                FluoMean(n)=DataSet.frames(h).cells.Stats(i).(strcat('MeanCellIntensity_',FluoProt));
