...
// @String(label="", description="Chose between: Li dark, IsoData dark, Otsu dark or Triangle (or Yen) dark (Brightfield)", value="Otsu dark") threshold
...
function cellAnalysis(Pole, IDiSCAT, IDReg, radius, AddCell, Discard_cell, Global_Stop, threshold, MinCellArea, RootMinCellArea, n, Brightfield, CellNb, RetractCount){
	...
	getSelectionCoordinates(x_BF, y_BF);
	...
	if(Brightfield){
		...
		selectImage(IDiSCAT);
		makeSelection( "polyline", x_BF, y_BF);
		roiManager("Add");
		...
		for (i = 0; i < 2; i++) {
			makeOval(x_BF[i],y_BF[i],radius*2,radius*2);
			waitForUser( "Pause","Count pili and flagella at the "+Pole[i]+" pole");
		
			Dialog.create("Pole "+Pole[i]+" at ("+x_BF[i]+", "+y_BF[i]+")");
		...
		setResult("X_Pole"+Pole[i], n, x_BF[i]);
		setResult("Y_Pole"+Pole[i], n, y_BF[i]);
		...
		if (RetractCount) {
				...
				for (k = 0; k < max_pili_nb; k++) {
					...
					for(i=0; i<2; i++){
						setResult("X_Pole"+Pole[i], n+k, x_BF[i]);
						setResult("Y_Pole"+Pole[i], n+k, y_BF[i]);
		...
		//Initializing variables for fluorescence analysis
		area = newArray(2);
		mean = newArray(2);
		min = newArray(2);
		max = newArray(2);
			...
			for(i=0; i<2; i++){
				...
				if (nResults>1) {
					...
					for (j = 0; j < nResults; j++) {
						x_ar[j]=getResult("X", j);
						y_ar[j]=getResult("Y", j);
					}
					Array.getStatistics(x_ar, min_temp, max_temp, x_cor, stdDev);
					Array.getStatistics(y_ar, min_temp, max_temp, y_cor, stdDev);
				} else {
					if (nResults==0) {
