...
// @Boolean(Label="Measure Fluorescence?", value=true) MesFluo
...
if(MesFluo){
	...
	while (Global_Stop == false){
		...
		//Checking Image quality and properties
		waitForUser( "Pause","Inspect Brightfield image");
		Dialog.create("Brightfield parameters");
			Dialog.addCheckbox("Discard event?", false);
			Dialog.addCheckbox("New Registration?", NewReg);
			Dialog.addCheckbox("Specific fluorescent frame", false);
			Dialog.addNumber("Fluorescent frame", 1);
		...
		if(!dropEvent){
			...
			//iSCAT and tirf imgaes registration
			IDReg=registrationBF(IDiSCAT, IDtirf, NewReg, JustFrame, FrameNb, Coordinates, dir, scaling);
			run("Tile");
			selectImage(IDReg);
			run("In [+]");
			selectImage(IDiSCAT);
			...
			while (AddCell==true){
				Discard_cell=false;
				n=nResults;
				Booleans=cellAnalysisBF(Pole, PiliCount, FlagellaCount, area, mean, min, max, std, TotalFluPole, IDiSCAT, IDReg, radius, AddCell, Discard_cell, Global_Stop, threshold, MinCellArea, RootMinCellArea, n);
				AddCell=Booleans[0];
				Global_Stop=Booleans[1];
			}
...
/*  cellAnalysisBF(Pole, PiliCount, FlagellaCount, area, mean, min, max, std, TotalFluPole, IDiSCAT, IDReg,
 *  radius, AddCell, Discard_cell, Global_Stop, threshold, MinCellArea, RootMinCellArea, n) Allows to
 *  automatically detect cell poles and record number of pili in each pole.
 * 
 */
function cellAnalysisBF(Pole, PiliCount, FlagellaCount, area, mean, min, max, std, TotalFluPole, IDiSCAT, IDReg, radius, AddCell, Discard_cell, Global_Stop, threshold, MinCellArea, RootMinCellArea, n){
	//Ask user to select cell of interest
	roiManager("Show All with labels");
	waitForUser( "Pause","Draw line along the cell starting from the dim to the bright pole");
	getSelectionBounds(x, y, SizeSquareX, SizeSquareY);
...
/*  registrationBF(IDiSCAT, IDtirf, NewReg, JustFrame, FrameNb, Coordinates, dir) takes the iSCAT and tirf imaege
 *  IDs as input as well as the booleans: NewReg for a new registration, JustFrame for a specific tirf frame to
 *  select, the frame number, the two lines coordinates, the scaling factor and the directory of the Tirf Image
 *  to save the registered result image and returns its ID.
 * 
 */
function registrationBF(IDiSCAT, IDtirf, NewReg, JustFrame, FrameNb, Coordinates, dir, scaling){
	selectImage(IDiSCAT);
	run("Gaussian Blur...", "sigma=2 stack");
	makeLine(Coordinates[0], Coordinates[1], Coordinates[2], Coordinates[3]);
